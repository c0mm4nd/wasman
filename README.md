...
| `EnableWideInt` | exposes the optional `u128`/`u256` import namespaces: 128/256-bit arithmetic over little-endian values in linear memory (EVM division conventions; `_s` variants cover i128/i256). Beyond the elementary ops, `u256.mul_div` computes `floor(a*b/c)` over the full 512-bit product (exact when `a*b >= 2^256`; `c==0` yields 0, a quotient past 256 bits traps) and `u256.isqrt` computes `floor(sqrt(a))` — the DeFi primitives (Uniswap-style mulDiv, geometric-mean minting), both allocation-free (~130ns / ~500ns). With the JIT, add/sub/mul/compare/bitwise/iszero compile to inline native code; div/rem/shifts/mul_div/isqrt dispatch reflection-free to the runtime |
